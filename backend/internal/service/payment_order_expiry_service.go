...
func (s *PaymentOrderExpiryService) runOnce() {
	...
	recovered, err := s.paymentSvc.ReconcileRecoverablePaymentOrders(reconcileCtx)
	cancel()
	if err != nil {
		slog.Warn("[PaymentOrderExpiry] failed to reconcile recoverable payment orders", "error", err)
	} else if recovered > 0 {
