...
// openAIStreamEventIsTerminal retains the generic helper name used by the
// passthrough and benchmark paths while Responses-specific callers use the
// more explicit openAIResponsesStreamEventIsTerminal name.
func openAIStreamEventIsTerminal(data string) bool {
	return openAIResponsesStreamEventIsTerminal(data)
}
...
// TempUnscheduleRetryableError 对 Google 配置错误触发临时封禁。
// 空流式响应属于上游瞬态故障，只进行同账号重试，不改变账号调度状态。
...
func (s *GatewayService) TempUnscheduleRetryableError(ctx context.Context, accountID int64, failoverErr *UpstreamFailoverError) {
	...
	if failoverErr.StatusCode == http.StatusBadGateway &&
		strings.Contains(strings.ToLower(string(failoverErr.ResponseBody)), "empty stream response") {
		return
	}
	// 根据状态码选择封禁策略
