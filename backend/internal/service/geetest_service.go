...
func (s *GeetestService) VerifyWithState(ctx context.Context, challenge GeetestChallenge) (bool, error) {
	...
	return s.VerifyWithConfig(ctx, GeetestCaptchaConfig{Enabled: enabled, CaptchaID: captchaID, CaptchaKey: captchaKey}, challenge)
}

// VerifyWithConfig verifies against the same provider-settings snapshot used
// to select GEETEST, avoiding a second settings read during authentication.
func (s *GeetestService) VerifyWithConfig(ctx context.Context, config GeetestCaptchaConfig, challenge GeetestChallenge) (bool, error) {
	if !config.Enabled {
		return false, nil
	}

	if config.CaptchaID == "" || config.CaptchaKey == "" || s.verifier == nil || s.replayGuard == nil {
	...
	claimed, err := s.replayGuard.Claim(ctx, config.CaptchaID, challenge.LotNumber, geetestReplayTTL)
	if err != nil {
	...
	result, err := s.verifier.Verify(ctx, config.CaptchaID, config.CaptchaKey, challenge)
	if err != nil {
