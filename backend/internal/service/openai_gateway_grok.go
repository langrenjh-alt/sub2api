...
func (s *OpenAIGatewayService) handleGrokAccountUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte) {
	...
	// Pool-mode still skips body-classified durable mutation unless an explicit
	// temporary rule matches. The classifier must run before this compatibility
	// gate so quota wording is not hidden by pool status handling.
	...
	if account.IsPoolMode() {
		// Pool accounts own member failover for configured retryable statuses and
		// upstream 5xx responses. Body-first quota classifications also stay
		// observational in pool mode. Preserve the legacy status fallback for a
		// bodyless, non-retryable response such as a bare 402 payment error.
		bodyClassified := decision.Class != GrokFailureNone && len(bytes.TrimSpace(responseBody)) > 0
		if shouldSkipGrokPoolErrorState(account, statusCode) || bodyClassified {
			slog.Info("grok_pool_mode_error_state_skipped", "account_id", account.ID, "status_code", statusCode, "classification", decision.Class)
			return
		}
	}
	switch statusCode {
