...
func TestSettingHandler_UpdateSettings_PreservesExistingGeetestCaptchaKey(t *testing.T) {
	...
	_, updated := repo.lastUpdates[service.SettingKeyGeetestCaptchaKey]
	require.False(t, updated)
	require.Equal(t, "existing-key", repo.values[service.SettingKeyGeetestCaptchaKey])
}

func TestSettingHandler_UpdateSettings_PreservesDisabledGeetestCaptchaIDOnPartialUpdate(t *testing.T) {
	gin.SetMode(gin.TestMode)
	repo := &settingHandlerRepoStub{values: map[string]string{
		service.SettingKeyGeetestEnabled:   "false",
		service.SettingKeyGeetestCaptchaID: "existing-captcha-id",
	}}
	svc := service.NewSettingService(repo, &config.Config{Default: config.DefaultConfig{UserConcurrency: 1}})
	handler := NewSettingHandler(svc, nil, nil, nil, nil, nil, nil)
	rawBody, err := json.Marshal(map[string]any{"registration_enabled": true})
	require.NoError(t, err)
	...
	require.Equal(t, http.StatusOK, rec.Code, rec.Body.String())
	require.Equal(t, "existing-captcha-id", repo.values[service.SettingKeyGeetestCaptchaID])
}
