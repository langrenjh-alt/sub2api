...
type AnnouncementQuery struct {
	...
	withComments *AnnouncementCommentQuery
...
func (_q *AnnouncementQuery) QueryComments() *AnnouncementCommentQuery {
...
func (_q *AnnouncementQuery) WithComments(opts ...func(*AnnouncementCommentQuery)) *AnnouncementQuery {
	query := (&AnnouncementCommentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AnnouncementQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Announcement, error) {
	...
	if query := _q.withComments; query != nil {
		if err := _q.loadComments(ctx, query, nodes,
			func(n *Announcement) { n.Edges.Comments = []*AnnouncementComment{} },
			func(n *Announcement, e *AnnouncementComment) { n.Edges.Comments = append(n.Edges.Comments, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AnnouncementQuery) loadComments(ctx context.Context, query *AnnouncementCommentQuery, nodes []*Announcement, init func(*Announcement), assign func(*Announcement, *AnnouncementComment)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Announcement)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(announcementcomment.FieldAnnouncementID)
	}
	query.Where(predicate.AnnouncementComment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(announcement.CommentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
