...
// The AnnouncementCommentFunc type is an adapter to allow the use of ordinary function as a Querier.
type AnnouncementCommentFunc func(context.Context, *ent.AnnouncementCommentQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f AnnouncementCommentFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.AnnouncementCommentQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.AnnouncementCommentQuery", q)
}

// The TraverseAnnouncementComment type is an adapter to allow the use of ordinary function as Traverser.
type TraverseAnnouncementComment func(context.Context, *ent.AnnouncementCommentQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseAnnouncementComment) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseAnnouncementComment) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.AnnouncementCommentQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.AnnouncementCommentQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.AnnouncementCommentQuery:
		return &query[*ent.AnnouncementCommentQuery, predicate.AnnouncementComment, announcementcomment.OrderOption]{typ: ent.TypeAnnouncementComment, tq: q}, nil
