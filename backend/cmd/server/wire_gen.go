...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	geetestVerifier := repository.NewGeetestVerifier()
	geetestReplayGuard := repository.NewGeetestReplayGuard(redisClient)
	geetestService := service.NewGeetestService(settingService, geetestVerifier, geetestReplayGuard)
	authService := service.ProvideAuthService(client, userRepository, redeemCodeRepository, refreshTokenCache, configConfig, settingService, emailService, turnstileService, emailQueueService, promoService, subscriptionService, affiliateService, serviceUserPlatformQuotaRepository, geetestService)
	...
	announcementCommentRepository := repository.NewAnnouncementCommentRepository(client)
	announcementService := service.ProvideAnnouncementService(announcementRepository, announcementReadRepository, announcementCommentRepository, userRepository, userSubscriptionRepository)
	announcementHandler := handler.NewAnnouncementHandler(announcementService)
	ticketRepository := repository.NewTicketRepository(client)
	ticketService := service.NewTicketService(client, ticketRepository, userRepository, settingService)
	...
	homepageStatusService := service.ProvideHomepageStatusService(groupRepository, channelMonitorRepository, settingService)
	handlerSettingHandler := handler.ProvideSettingHandler(settingService, buildInfo, notificationEmailService, homepageStatusService)
