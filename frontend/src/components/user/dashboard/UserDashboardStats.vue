  ...
  <div v-if="!isSimple && platformCards.length > 0" class="card p-4">
    ...
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
      ...
      >
        ...
        <!-- Quota 區：僅當 quota 配置存在、非 __other__ 且至少有一個視窗配了 limit 時顯示 -->
...
// 處理"各平台之和 < 總值"的差值：後端按平台聚合時過濾了無法歸屬平台的行
// （group 與 account 都缺 platform）。這裡把差值作為"其他"卡片顯式展示，
// 避免 Row 1 總值與 Row 3 平台拆分加總對不上、使用者困惑。
...
const platformCards = computed<FusedPlatformCard[]>(() => {
  ...
  // union 平台集合。後端 by_platform / quota 介面均不會返回 platform='__other__'，
  // 無需顯式排除；__other__ 由下方差值補差邏輯單獨追加。
  ...
  // __other__ 補差邏輯：只對 by_platform 有 usage 資料的總和計算
...
// 與 formatBalance 一致使用 Intl.NumberFormat 做半偶舍入，避免 toFixed 在不同 JS 引擎
// 下偶發截斷而非四捨五入（與後端展示精度不一致）。
