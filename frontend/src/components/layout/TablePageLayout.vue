...
.table-page-layout {
  ...
  height: calc(100vh - 64px - 4rem); /* 減去 header + lg:p-8 的上下padding */
}
...
.table-scroll-container :deep(table) {
  @apply w-full;
  min-width: max-content; /* 關鍵：確保表格寬度根據內容撐開，從而觸發橫向滾動 */
  display: table; /* 使用標準 table 佈局以支援 sticky 列 */
}
...
.table-scroll-container :deep(tbody) {
  /* 保持預設 table-row-group 顯示，不使用 block */
}
