...
/**
 * 搜尋過濾：
 * - 命中渠道名/描述 → 整個渠道（所有 platforms）都保留
 * - 否則按 platform/group/model 維度在 sections 裡過濾，保留有匹配的 section
 * - 所有 sections 都不匹配時，渠道本身被過濾掉
 */
...
async function loadChannels() {
  loading.value = true
  try {
    // 渠道列表和使用者專屬倍率並行拉取。專屬倍率失敗不阻塞渠道展示——
    // 失敗時只是無法渲染專屬倍率角標，降級為僅顯示預設倍率。
