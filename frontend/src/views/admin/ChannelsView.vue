  ...
  for (const section of form.platforms.filter(s => s.enabled)) {
    ...
    for (const entry of section.model_pricing) {
      if (entry.models.length === 0) {
        const platformLabel = t('admin.groups.platforms.' + section.platform, section.platform)
        appStore.showError(t('admin.channels.emptyModelsInPricing', { platform: platformLabel }, `${platformLabel} 平台下有定价条目未新增模型，请新增模型或删除该条目`))
        activeTab.value = section.platform
        return
      }
    }
      ...
      if (mappingConflict) {
        appStore.showError(
          t('admin.channels.mappingConflict',
            ...
            `模型对映源 '${mappingConflict[0]}' 和 '${mappingConflict[1]}' 冲突：匹配范围重叠`)
        )
        activeTab.value = section.platform
        return
      }
