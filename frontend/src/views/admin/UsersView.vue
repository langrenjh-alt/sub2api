    ...
    <TablePageLayout>
      ...
      <template #table>
        ...
        >
          ...
          <!-- 用量列自定义表头：列名 + 单个排序图示按钮，点击展开"今日/近30天"菜单。
               column.sortable=false，DataTable 内建点击逻辑不会触发；
...
// usage 主列或任意 usage_<platform> 子列可见时都需要批次拉取用量资料
...
// 字段都会触发后端查询，而用量列资料是非同步批次拉取后再合并到当前页，
// 因此采用独立的前端排序状态对当前页 users 做本地排序。
// 排序状态独立于后端 sortState 持久化；缺失资料按 0 处理（desc 沉底、asc 置顶）。
...
// 列头排序按钮点击后弹出的"今日/近30天"选择菜单，同时只允许一个列展开。
// 点击图示本身不触发排序，仅开关菜单；首次排序由用户在菜单内选择 metric 触发（默认 desc，详见 toggleUsageSort）。
...
// 稳定排序：等值按原 index 保序，避免拉取新用量资料时表行抖动。
...
const loadUsersSecondaryData = async (
  ...
  if (hasVisiblePlatformQuotaColumn.value) {
    tasks.push(
      (async () => {
        try {
          // 无批次端点：对当前页用户逐个拉取，分块并行（每批 6），批间检查中止条件，避免大 pageSize 时请求洪峰
          const CHUNK = 6
